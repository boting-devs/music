...
__all__ = ("AlreadyPaused", "AlreadyResumed", "NotInRange", "InvalidFormat")
...
class NotInRange(CheckFailure):
    embed = ErrorEmbed(
        title="Not in Range", description="Amount cannot be more than song's length"
    )


class InvalidFormat(CheckFailure):
    embed = ErrorEmbed(
        title="Invalid Format", description="Please use HH:MM:SS or MM:SS format"
    )
