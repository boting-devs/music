...
class Looptrack(CogBase[Vibr]):
    ...
    async def looptrack(self,inter:MyInter):
        ...
        if not player.loop_track:
            ...
            embed = Embed(title="Loop Mode On",description=f"{inter.user.mention} turned on loop mode",timestamp=utcnow())
            await inter.send(embed=embed)
            
        
        else:
            player.loop_track = None
            embed = Embed(title="Loop Mode Off",description=f"{inter.user.mention} turned off loop mode",timestamp=utcnow())
